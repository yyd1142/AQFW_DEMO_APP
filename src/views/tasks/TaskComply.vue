<template>
    <div class="taskComply">
        ...
        <mko-header title="任务执行" left-icon="icon-back" right-icon-text="执行记录" @handleRightClick="linkPath('/taskHistory')" @handleLeftClick="back" v-if="isAdmin"></mko-header>
        <mko-header title="任务执行" right-icon-text="执行记录" @handleRightClick="linkPath('/taskHistory')" v-else></mko-header>
        <div class="page-wrap">
                        ...
                        <div class="tasks-item" v-for="cell, index in xunchaTasks" @click="viewXunchaTask(cell)">
                            ...
                            <div class="account">
                                <div class="item" v-if="cell.isRoutine && cell.isRoutine == 1">
                                    <span style="color: #3399FF" v-if="cell.executedName">{{cell.executedName}}正在进行中</span>
                                    <span :style="{color: xcTaskStyleFilter(cell.status)}" v-else>{{cell.status | taskStatusFilter}}</span>
                                </div>
                    ...
                    <div class="padding">
                        <div class="tasks-item" v-for="cell, index in zhibanTasks" @click="viewZhibanTask(cell)">
                            ...
                            <span class="task-type">
                                ...
                                <span class="time" v-if="cell.type == 0">{{cell.endTime | formatDate('YYYY-MM-DD')}} 截止</span>
                            </span>
                            <div class="account">
                                <div class="item">
                                    <span :style="{color: zbTaskStyleFilter(cell.status)}">{{cell.status | taskStatusFilter}}</span>
                                </div>
                                <i class="icon icon-arrow-right"></i>
                            </div>
                        </div>
