<template>
    <div class="CreateZhibanTable">
        <div class="placeholder-item"></div>
        <mko-header title="新建值班表" left-icon="icon-back" @handleLeftClick="back" right-icon-text="保存" @handleRightClick="save"></mko-header>
        <div class="page-wrap">
            <div class="form-card">
            ...
            <mko-edit-card :title="'班次'+ (index + 1)" title-color="black-font" :edit="false" :remove="true" @remove="removeZhibanCard(index)" v-for="(cell, index) in formData.banciDatas">
                <mko-form-cell title="班次名称" :edit="true" type="text" v-model="cell.bcName" holder-text="填写班次名称, 例 : 早班"></mko-form-cell>
        ...
        return {
            ...
            formData: {
                description: '', xiuxiri: '', zbDescribe: '', banciDatas: [{
                    bcName: '', bcTime: '', executor: '', executorName: ''
                }]
            },
            weekList: [
                { text: '周一', value: 1 },
    ...
    methods: {
        ...
        selPickerVal() {
            if (this.pickerWrapperName === 'zhibanTime') {
                ...
                let offDuty = []
                this.offDutyDatas.forEach(data => { offDuty.push(week[data]) })
        ...
        save() {
            ...
            for (let key in this.formData) {
                if(key != 'xiuxiri') {
                    if (!this.formData[key]) isEmpty = true;
                }
            }
