<template>
    <div class="EditTmpZhiban">
        <div class="placeholder-item"></div>
        <mko-header title="编辑值班表" left-icon="icon-back" @handleLeftClick="back" right-icon="icon-delete" @handleRightClick="remove"></mko-header>
        <div class="page-wrap">
            <div class="form-card">
                <mko-form-cell title="值班表名称" :edit="true" type="text" v-model="formData.description" holder-text="请输入值班表名称"></mko-form-cell>
                <mko-textarea :val="formData.zbDescribe" v-model="formData.zbDescribe" placeholder="这里是值班表描述"></mko-textarea>
            </div>
            <div class="form-card last-child">
                <mko-form-cell title="休息日" :val="formData.xiuxiri || '选择'" v-model="formData.xiuxiri" :edit="true" type="sel" @click="popupPickerShow('offDuty')"></mko-form-cell>
            ...
            <mko-edit-card :title="'班次'+ (index + 1)" title-color="black-font" :edit="false" :remove="true" @remove="removeZhibanCard(cell, index)" v-for="(cell, index) in formData.banciDatas">
                <mko-form-cell title="班次名称" :val="cell.bcName" v-model="cell.bcName" :edit="true" type="text" holder-text="填写班次名称, 例 : 早班"></mko-form-cell>
        ...
        <mko-popup-bottom :title="popupName" v-model="selPopupShow" @save="selPickerVal" @cancel="selPopupClose">
            ...
            <mko-select-box :column="4" :options="weekList" :selected="offDutyDatas" @select="onOffDutyChange" v-if="pickerWrapperName === 'offDuty'"></mko-select-box>
...
export default {
    ...
    activated() {
        formData = this.$route.query.bcDetail;
        this.$nextTick(() => {
    ...
    deactivated() {
        this.formData = {
            description: '', xiuxiri: '', zbDescribe: '', banciDatas: []
        }
        this.zhibanTime = ''
        this.popupName = ''
        this.offDutyDatas = [];
    ...
    methods: {
        ...
        addBanci() {
            ...
            this.formData.banciDatas.push({ bcName: '', bcTime: '', executorName: '', executor: '' });
        },
        ...
        selPickerVal() {
            if (this.pickerWrapperName === 'zhibanTime') {
                ...
                this.formData.banciDatas[this.zhibanIndex].executor = this.zhibanNamesSelecteds.join(',');
